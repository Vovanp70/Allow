...
// Видимость карточек Stubby-family и Dnsmasq-full family на дашборде (по настройке «Фильтрация контента для детей»)
async function loadChildrenFilterVisibility() {
    const stubbyCard = document.getElementById('dashboard-card-stubby-family');
    const dnsmasqCard = document.getElementById('dashboard-card-dnsmasq-family');
    if (!stubbyCard && !dnsmasqCard) return;
    try {
        const data = await apiRequest('/settings/children-filter');
        const enabled = !!(data && data.enabled === true);
        const display = enabled ? '' : 'none';
        if (stubbyCard) stubbyCard.style.display = display;
        if (dnsmasqCard) dnsmasqCard.style.display = display;
    } catch (err) {
        if (stubbyCard) stubbyCard.style.display = '';
        if (dnsmasqCard) dnsmasqCard.style.display = '';
    }
}

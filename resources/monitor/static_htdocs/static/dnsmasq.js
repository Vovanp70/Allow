...
async function loadDnsmasqStatus() {
    ...
    try {
        ...
        if (!autostartActive && !running) {
            ...
            statusElement.className = 'value status-disabled';
        } else if (running) {
        ...
        const toggleBtn = document.getElementById('dnsmasq-full-toggle-autostart-btn') || document.getElementById('dnsmasq-toggle-autostart-btn');
        if (toggleBtn) {
            toggleBtn.textContent = autostartActive ? 'Отключить' : 'Включить';
        }

        const restartBtn = document.getElementById('dnsmasq-full-restart-btn') || document.getElementById('dnsmasq-restart-btn');
        if (restartBtn) {
            restartBtn.style.display = (!autostartActive && !running) ? 'none' : 'inline-block';
        }
...
// Включить/отключить автозапуск DNSMASQ full
async function toggleDnsmasqAutostart() {
    const toggleBtn = document.getElementById('dnsmasq-full-toggle-autostart-btn') || document.getElementById('dnsmasq-toggle-autostart-btn');
    if (toggleBtn) toggleBtn.disabled = true;
    try {
        const statusData = await apiRequest('/dnsmasq/status');
        const autostartActive = statusData && statusData.autostart_active === true;
        const action = autostartActive ? 'deactivate' : 'activate';
        showProgress(autostartActive ? 'Отключение DNSMASQ...' : 'Включение DNSMASQ...');
        const data = await apiRequest('/dnsmasq/autostart', 'POST', { action });
        if (data.success) {
            showToast(data.message || (autostartActive ? 'DNSMASQ отключен' : 'DNSMASQ включен'));
            await loadDnsmasqStatus();
        } else {
...
async function loadDnsmasqFamilyStatus() {
    ...
    try {
        ...
        const toggleBtn = document.getElementById('dnsmasq-family-toggle-autostart-btn');
        if (toggleBtn) {
            toggleBtn.textContent = autostartActive ? 'Отключить' : 'Включить';
        }
...
// Включить/отключить автозапуск DNSMASQ Family
async function toggleDnsmasqFamilyAutostart() {
    const toggleBtn = document.getElementById('dnsmasq-family-toggle-autostart-btn');
    if (toggleBtn) toggleBtn.disabled = true;
    try {
        const statusData = await apiRequest('/dnsmasq-family/status');
        const autostartActive = statusData && statusData.autostart_active === true;
        const action = autostartActive ? 'deactivate' : 'activate';
        showProgress(autostartActive ? 'Отключение DNSMASQ Family...' : 'Включение DNSMASQ Family...');
        const data = await apiRequest('/dnsmasq-family/autostart', 'POST', { action });
        if (data.success) {
            showToast(data.message || (autostartActive ? 'DNSMASQ Family отключен' : 'DNSMASQ Family включен'));

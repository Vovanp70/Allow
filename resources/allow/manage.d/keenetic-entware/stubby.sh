#!/bin/sh
#
# Управление Stubby для монитора: status, start, stop, restart.
# Вывод в формате KEY=value (по одной строке) для разбора CGI.
# Использование: stubby.sh status | start | stop | restart
#
...
get_kv() {
    key="$1"
    [ -x "$STUBBY_INIT" ] || return 1
    out="$("$STUBBY_INIT" status --kv 2>/dev/null)" || true
    line="$(echo "$out" | grep "^${key}=" | head -1)"
    [ -n "$line" ] && echo "${line#*=}" && return 0
    return 1
}

# --- Проверка процесса stubby (pid или ps) ---
is_stubby_running() {
    if [ -f "$STUBBY_PID" ]; then
        pid="$(cat "$STUBBY_PID" 2>/dev/null | tr -d '\r\n')"
        if [ -n "$pid" ] && [ -d "/proc/$pid" ]; then
            cmd="$(cat /proc/$pid/cmdline 2>/dev/null | tr '\0' ' ')"
            case "$cmd" in *stubby*stubby.yml*) return 0 ;; esac
        fi
    fi
    ps w 2>/dev/null | grep -v grep | grep "stubby" | grep -q "stubby.yml" && return 0
    return 1
}
...
cmd_status() {
    ...
    if is_stubby_running; then
        STATUS="running"
        [ -z "$PID" ] && [ -f "$STUBBY_PID" ] && PID="$(cat "$STUBBY_PID" 2>/dev/null | tr -d '\r\n')"
        [ -z "$PID" ] && PID="$(ps w 2>/dev/null | grep -v grep | grep 'stubby.*stubby.yml' | awk '{print $1}' | head -1)"
    else
        STATUS="stopped"
    fi
...
cmd_start() {
    if is_stubby_running; then
        echo "SUCCESS=no"
        echo "MESSAGE=Stubby already running"
        exit 0
    fi
    if [ -f "$STUBBY_CONFIG" ] && command -v stubby >/dev/null 2>&1; then
        if ! stubby -C "$STUBBY_CONFIG" -i >/dev/null 2>&1; then
            echo "SUCCESS=no"
            echo "MESSAGE=Stubby configuration is invalid"
            exit 0
        fi
    fi
    if [ -x "$STUBBY_INIT" ]; then
        "$STUBBY_INIT" start >/dev/null 2>&1 || true
        sleep 2
    fi
    if is_stubby_running; then
        echo "SUCCESS=yes"
        echo "MESSAGE=Stubby started successfully"
    else
        echo "SUCCESS=no"
...
cmd_stop() {
    if ! is_stubby_running; then
        echo "SUCCESS=no"
        echo "MESSAGE=Stubby is not running"
        exit 0
    fi
    [ -x "$STUBBY_INIT" ] && "$STUBBY_INIT" stop 2>/dev/null || true
    sleep 2
    if is_stubby_running; then
        pids="$(pgrep -f 'stubby.*stubby.yml' 2>/dev/null)" || true
        for p in $pids; do
    ...
    if ! is_stubby_running; then
        echo "SUCCESS=yes"
        echo "MESSAGE=Stubby stopped successfully"
    else
        echo "SUCCESS=no"
...
cmd_restart() {
    was_running=0
    is_stubby_running && was_running=1
    [ -x "$STUBBY_INIT" ] && "$STUBBY_INIT" stop 2>/dev/null || true
    sleep 2
    [ -x "$STUBBY_INIT" ] && "$STUBBY_INIT" start 2>/dev/null || true
    sleep 2
    if is_stubby_running; then
        echo "SUCCESS=yes"
        echo "MESSAGE=Stubby restarted successfully"
    else
        echo "SUCCESS=no"
        echo "MESSAGE=Failed to start"
    fi
